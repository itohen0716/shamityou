(() => {
  ...
  const scriptUrl = document.currentScript?.src || new URL("./tuning-data.js", document.baseURI).href;
  const dataUrl = new URL("./tuning-master.json", scriptUrl).href;

  window.ShianTuningMasterReady = fetch(dataUrl, { cache: "no-cache" })
    .then((response) => {
